  ...
  fetch(detailsUrl)
    ...
    .then(data => {
      ...
      seasonSelect.addEventListener('change', () => {
        ...
        updateUrl(id, mediaType, selectedSeason, selectedEpisode);
      });
...
function displayDetails(details, mediaType, initialSeason, initialEpisode) {
  ...
  if (mediaType === 'movie') {
    ...
    const seasonNumber = initialSeason || seasonSelect.value;
    const episodeNumber = initialEpisode || episodeSelect.value;
    updateEpisodeDetails(details.id, seasonNumber, episodeNumber);
    watchNowButton.addEventListener('click', () => openVideo(details.id, 'tv', seasonNumber, episodeNumber));
  }
}

function openVideo(id, mediaType, seasonNumber = '', episodeNumber = '') {
...
function setupSeriesOptions(tvDetails, initialSeason, initialEpisode) {
  ...
  seasonSelect.value = initialSeason || (regularSeasons.length ? regularSeasons[0].season_number : '');
